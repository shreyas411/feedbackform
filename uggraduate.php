...
if(isset($_POST['sub']))
{
    ...
    {
        ?>
       <form action="exportuggraduate.php" method="post">
       <input name="download" type="submit" value="Download" style="margin-left:50px; margin-top:50px;">
        </form>
        <?php

         $sql = "SELECT * FROM `paper` where cname = '$cname' && cyear = '$cyear' && sem = '$sem' &&  dept = '$dept'"; #select query where resource person name
